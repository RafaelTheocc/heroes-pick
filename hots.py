...
for mapa in url:
    ...
    dict = {}
    x=0
    map_element = driver.find_element_by_xpath('/html/body/div/div[2]/div[1]/div[2]/div/h1')
    map_content = map_element.get_attribute('innerHTML')
    
    while True:
        x+=1
        try:
            pers_element = driver.find_element_by_xpath('/html/body/div/div[2]/ul/li['+str(x)+']/div/h3/a')
            personagem_name = pers_element.get_attribute('innerHTML')

            element = driver.find_element_by_xpath('/html/body/div/div[2]/ul/li['+str(x)+']/div/a/span[2]/strong')
            html_content = element.get_attribute('innerHTML')
            html_content1 = int(html_content)
            dict[personagem_name] = [html_content1]
        except:
            break
